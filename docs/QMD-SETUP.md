...
\`\`\`bash
qmd search "<topic>" -c tenant
# Cerca solo in ./laravel/Modules/Tenant/docs/wiki/
\`\`\`
...
\`\`\`bash
qmd search "form" -c tenant  # Solo questo modulo
\`\`\`
...
| Ricerca lenta | Limita con `-c tenant` |
